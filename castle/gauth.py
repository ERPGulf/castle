...
def getToken(self):
		try:
			...
			if user and frappe.utils.password.check_password(user.password, password):
				...
				frappe.cache().hset("user_tokens", token, user.name)  # Store the token in Redis cache
...
def generate_custom_token(username, password):
    try:
        ...
        client_id = "5fed5aada3"  # Replace with your OAuth client ID
        client_secret = "708c75201b"  # Replace with your OAuth client secret
        url = "https://castle.erpgulf.com:1813/api/method/castle.gauth.get_token"
